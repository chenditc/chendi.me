...
同时在这个过程中也发现了一些面试题的影子，所以说“面试造火箭，工作拧螺丝”并不正确，面试的题目也可以反映在工作中遇到复杂问题时，是否有独自搞明白的能力。
...
7. 设计取舍
 - Apache Ozone 的架构相对于 Azure 和 GFS + Bigable 简单一些，因为在对象 key 映射到底层 Block 的这一层 Ozone 只有一个 Ozone Manager 进行处理。所以 Ozone Manager 相当于整个系统的一个**单点性能瓶颈**。
...
1. 设计目标：
 - 利用计算节点的自治性，提供去中心化的高吞吐量低延迟的对象储存
