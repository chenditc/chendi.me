...
SSTable 是在 BigTable paper 中提到的用来讲数据持久化在磁盘上的最小数据单元。他将内存中的数据根据 key 排序后，序列化写入一段连续的空间内。并且把每一个 key 对应的 offset 写入到最末尾的 index 空间中。

这样打包写入数据的方式，对比一个数据写入一个文件的方式有几个优点：
...
当有新数据需要存储时，可能需要 master 将这个数据块注册到元信息中，所以这样存储的信息，往往不会是小文件，从而避免元信息太大。

同时，由于 master 操控了所有 slave 的负载分配，也可以做出不少负载均衡上的优化，例如：
1. 当某个 slave 的存储空间快满了，可以减少它存储的新数据，同时迁移一部分数据
2. 当某个 slave 上出现热点数据是，可以增加热点数据的备份，使得多个 slave 可以同时 serve 一份数据。

主从结构是许多系统采用的他本身有许多好处：
1. 架构清晰明了，元信息的处理都在主机发生。
...
2. 读写数据时需要先向 master 请求元信息，可能会增加读写数据时的延迟和资源开销。这一点往往需要客户端进行缓存来进行优化。
...
1. 使用一致性哈希 + 数据迁移的方法。[一致性哈希](https://en.wikipedia.org/wiki/Consistent_hashing)算法保证了当哈希表的大小变化后，需要变化的值的数量只有 n/m 个，n 是 key 的数量，m 是哈希表的大小，即把需要迁移的 key 数量降到最低，从而减少系统迁移数据的负载。使用一致性哈希时往往也会使用一个物理节点映射到多个哈希空间中的虚拟节点的方法，从而使得节点添加和移除时，负载可以均匀地被分配到其他节点上。
2. 使用多备份或者 Erasure Coding 保证集群内的容灾性能，但不支持对集群的扩容和缩容，只支持跨集群的负载均衡。就是说在集群内如果有少量机器挂了，不影响集群内的读写操作。如果容量满了，则可以触发跨集群的负载均衡，将新写入的数据转发至其他集群进行处理。
...
2. 负载均衡依赖于哈希算法本身的优越性，难以对负载进行灵活调整。
3. 热点数据难以扩展

#### 拜占庭将军问题

TODO：补充不同系统解决network partition 的方法
