...
在上一篇博客里，我提到了一个众包数据集的项目，是我作为 AI School 的项目贡献给 Qlib 社区的。这篇博客大概记录一下我做这个项目的动机、过程以及未来的计划。

[![perf_analysis_1.png](/img/in-post/quant/perf_analysis_1.png)](/img/in-post/quant/perf_analysis_1.png)

在尝试了 Qlib 的示例代码之后，Qlib 的回测结果显示在 2017 - 2020年间的回测年化扣费超额收益可以达到 10% 以上。

我下一步好奇的便是：如果把这个模型在 2020 年投入实盘的话，这两年的效果如何？
...
Qlib 的 A 股数据集来源经过 3 个阶段：
 - 第一个阶段是爬虫将每个股票的数据采集下来，每个股票作为一个 csv 文件。
 - 第二个阶段是处理这些 csv 文件，使得价格等数据归一化，存成新的 csv 文件。这一步是不可逆的，因为归一化之后所有股票第一日的价格都是 1，我们无法得知原本的股价是多少。
 - 第三个阶段是将归一化后的 csv 文件转成 numpy 格式的 bin 文件。

如果要对数据源进行更新，就需要将获得第一阶段中生成的 csv 文件，然后在这个基础上添加新数据并更新。但第一阶段的数据并没有提供，所以在原来的 demo 数据集上更新的方案便行不通了。只能从头爬取所有股票数据，然后对数据重新进行一边处理。

但是在尝试重新爬取数据时，又发现了一些其他问题。

### Yahoo Finance 数据源质量问题

Yahoo Finance 的数据源本身质量也不高：
...
为了能获得高质量的回测数据集，我们最好能利用多个不同的数据集进行交叉验证。
...
为了能获得持续更新的指数成分股数据，我们只能从其他途径获取到历史的变更信息，然后再从公告信息中拉去最新数据进行合并。

### 数据领域的重复劳动

类似上述的数据源质量的问题并不是我一个人会遇到的，那为什么每个遇到的人都要解决这样的问题呢？

在数据领域，即便是公开数据，例如国家统计局发布的 GDP、人口数据，股票数据，或者财报数据，都有可能会有数据错误，也都需要做数据的校验。
...
为了解决 Qlib 的数据持续更新问题，我决定发起一个众包项目。由我提供初始版本的数据，并做一些基础的校验。后续如果开源社区有兴趣提高 Qlib 的数据集，或者导出作为其他项目的数据集，都可以节约一些时间。
...
```
如果是导入到一个新表，Dolt 会自动根据字短的内容创建对应的字段类型，这个流程和 Spark 导入 csv 数据有点像。
...
### 提交数据commit
和 Git 类似，Dolt 也可以添加数据的 commit，通过 commit 的方式来管理数据的修改：
...
有了历史之后，我们甚至可以查看某条数据的某个 cell 是被谁在哪个 commit 修改的：
```sql
SELECT diff_type, `from_link_date`, `to_link_date`, from_commit, from_commit_date, to_commit, to_commit_date
FROM `dolt_diff_a_stock_eod_price`
WHERE (`to_w_symbol` = "BJ430198" OR `from_w_symbol` = "BJ430198") AND (`from_link_date` <> `to_link_date` OR (`from_link_date` IS NULL AND `to_link_date` IS NOT NULL) OR (`from_link_date` IS NOT NULL AND `to_link_date` IS NULL))
...
校验过程中用到的 sql 和代码则托管在 Github 上，尽量让过程可以回溯。

最后我设置了一个 Github Action，每日从公开数据集导入数据，并导出 Qlib 格式的 bin 文件，打包成 Artifact 上传到 Github 上。这样一来，如果有 Qlib 最新数据需求的同学，只要下载解压最新的 Github Release 即可。
...
3. 数据校验算法和性能优化。目前的校验主要是通过 SQL 来实现的，部分操作例如多表的 Join 和聚合统计比较慢。最好可以进行优化一下，从而部署到 Github Actions 之类的 CI 平台，利用平台算力进行校验。
4. 接入更多公开数据源。接入多个数据源的目的是希望可以尽量避免单数据源导致的 Single Point of Failure，减少数据错误和延迟的风险。
...
我最开始的时候是为了检验 Qlib 模型在近两年的表现如何，延长回测时间后发现，在 2021.01.01 至 2022.08.01 模型在沪深 300 的股票池上仍然可以提供 36% 的无费率年化超额，15.2% 的无换手限制扣费年化超额：
...
**优点：**
 ...
 - Dolt clone 和 push 提供了快捷的批量数据同步机制。

**缺点：**
 - 在大数据量的场景下，Dolt 的性能和 SQL 数据库有几倍的差距，同时难以利用集群进行加速，TB 级别的数据不太适用。
 - 无法自动保存 SQL，对于某个 commit 来讲，可以查询到修改了哪些数值，但是没办法判断这个数值是怎么来的。
 - 如果不需要全量数据的话，dolt clone 时不能只 clone 某张表。

**适用场景：**
 - GB 或以下级别的数据量，数据表之间强关联，大部分时候会同时使用到。例如商品信息数据，公司信息数据。
 - 修改时以手动修改为主的数据。

**不适用场景：**
 - TB 级别的数据。
 - 数据表间相关性不大，大部分之后只用到 10% 以下的数据量。
 - 数据修改主要通过脚本或 SQL 进行大批量的修改，例如用户行为统计数据。

希望这个总结也可以在后续数据项目的选型提供一些帮助。
